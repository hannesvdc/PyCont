...
def pseudoArclengthContinuation(G, Gu, Gp, u0, p0, ds_min, ds_max, ds, N, tolerance=1.e-10):
    rng = rd.RandomState()
    M = u0.size

    # Setting up variables for continuation
    result = ContinuationResult()
    tangent = pac.computeTangent(Gu(u0, p0), Gp(u0, p0), rng.normal(0.0, 1.0, M+1), M, tolerance)

    # Do continuation in both directions of the tangent
    _recursiveContinuation(G, Gu, Gp, u0, p0,  tangent, ds_min, ds_max, ds, N, tolerance, result)
    _recursiveContinuation(G, Gu, Gp, u0, p0, -tangent, ds_min, ds_max, ds, N, tolerance, result)

    # Return all found branches and bifurcation points
    return result

def _recursiveContinuation(G, Gu, Gp, u0, p0, tangent, ds_min, ds_max, ds, N, tolerance, result):
    M = u0.size
    ...
    # If there are no addittional bifurcation points on this path, return
    if len(bf_points) == 0:
       return
    ...
    for n in range(len(result.bifurcation_points)):
        if lg.norm(x_singular - result.bifurcation_points[n]) < 1.e-4:
            return
